...
export const handlers = [
    ...
    rest.post("/todos", (req, res, ctx) => {
        ...
        const newRefId = refId.flatMap((id) =>
            data.messages.filter((todo) => todo.id === id)
        );
        const newMessage = {
            id: id,
            title: title,
    ...
    rest.put("/todos/:id", (req, res, ctx) => {
        ...
        const { newTitle, isCompleted, refId } = body;
        ...
        if (todoToUpdate) {
            if (refId && refId.length > 0) {
                ...
                todoToUpdate.refId = data.messages.filter((todo) =>
                    refId.find((refTodo) => refTodo.id === todo.id)
                );
